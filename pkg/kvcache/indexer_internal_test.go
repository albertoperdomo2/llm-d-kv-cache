...
func TestValidateStorageConfig(t *testing.T) {
	...
	t.Run("accumulator capacity must be positive", func(t *testing.T) {
		cfg := DefaultStorageConfig()
		cfg.Enabled = true
		cfg.AccumulatorCapacity = 0
		if err := validateStorageConfig(cfg); err == nil {
			t.Fatal("expected zero accumulator capacity to fail validation")
		}
	})

	t.Run("gpu token cache capacity must be positive", func(t *testing.T) {
		cfg := DefaultStorageConfig()
		cfg.Enabled = true
		cfg.GPUTokenCacheCapacity = 0
		if err := validateStorageConfig(cfg); err == nil {
			t.Fatal("expected zero gpu token cache capacity to fail validation")
		}
	})
}
