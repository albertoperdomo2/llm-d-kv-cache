...
def _vllm_config_symbols():
    try:
        from vllm.config import VllmConfig, set_current_vllm_config
    except (
        ImportError
    ):  # pragma: no cover - exercised in lightweight unit test environments
        return None, None
    return VllmConfig, set_current_vllm_config


@pytest.fixture(scope="session", autouse=True)
def require_cuda(request):
    """Skip only tests that explicitly require CUDA/vLLM."""
    if request.node.get_closest_marker("requires_cuda") is None:
        return
    VllmConfig, set_current_vllm_config = _vllm_config_symbols()
